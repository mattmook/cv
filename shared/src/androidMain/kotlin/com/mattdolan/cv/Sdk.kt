...
actual class Sdk(private val context: Context) {

    actual fun profileRepository(): ProfileRepository {
        val settings = AndroidSettings.Factory(context).create()
        val cvNetworkDataSource = CvNetworkDataSourceImpl()
        val appDatabase = AppDatabase(DriverFactory(context).createDriver())
